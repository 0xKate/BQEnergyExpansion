...
public class ESSGuiContainer extends GuiContainerCanvas implements INeedsRefresh {
    private final ESSContainer essContainer;
    ...
    public ESSGuiContainer(GuiScreen parent, InventoryPlayer playerInvo, ESSTileEntity submitStation) {
        super(parent, new ESSContainer(playerInvo, submitStation));
        this.essContainer = (ESSContainer) this.inventorySlots;
    ...
    public void initPanel() {
        ...
        PanelTextBox txtTitle = new PanelTextBox(
                        ...
                        QuestTranslation.translate("bqenergy.gui.ess.title"))
                .setAlignment(1);
        ...
        btnSet = new PanelButton(new GuiTransform(new Vector4f(0.75F, 0F, 0.75F, 0F), -16, 64, 16, 16, 0), -1, "") {
            ...
            public void onButtonClick() {
                ...
                ESSPacketHandler.setupStation(
                        tile.xCoord,
                        tile.yCoord,
                        tile.zCoord,
                        QuestDatabase.INSTANCE.getID(quest),
                        quest.getTasks().getID(task));
                refreshTaskPanel();
            }
        };
        ...
        btnRem = new PanelButton(new GuiTransform(new Vector4f(0.75F, 0F, 0.75F, 0F), 0, 64, 16, 16, 0), -1, "") {
            ...
            public void onButtonClick() {
                tile.reset();
                ESSPacketHandler.resetStation(tile.xCoord, tile.yCoord, tile.zCoord);
                refreshTaskPanel();
            }
        };
    ...
    private void setInventoryPosition(int x, int y) {
        ...
        essContainer.moveInventorySlots(x + 1, y + 23);
        essContainer.moveSubmitSlot(x + 55, y + 1);
        essContainer.moveReturnSlot(x + 91, y + 1);
        ...
        for (int i = 0; i < 36; i++) {
            ...
            this.addPanel(new PanelGeneric(
                    new GuiTransform(GuiAlign.TOP_LEFT, x + j, y + k + 22, 18, 18, -1),
                    PresetTexture.ITEM_FRAME.getTexture()));
        }

        this.addPanel(new PanelGeneric(
                new GuiTransform(GuiAlign.TOP_LEFT, x + 54, y, 18, 18, -1), PresetTexture.ITEM_FRAME.getTexture()));
        this.addPanel(new PanelGeneric(
                new GuiTransform(GuiAlign.TOP_LEFT, x + 72, y, 18, 18, -1), PresetIcon.ICON_RIGHT.getTexture()));
        this.addPanel(new PanelGeneric(
                new GuiTransform(GuiAlign.TOP_LEFT, x + 90, y, 18, 18, -1), PresetTexture.ITEM_FRAME.getTexture()));
    }
    ...
    private void filterQuests() {
        ...
        while (iter.hasNext()) {
            ...
            for (DBEntry<ITask> tsk : entry.getValue().getTasks().getEntries()) {
                if (tsk.getValue() instanceof IEUTask) {
                    valid = true;
                    break;
                }
            }
    ...
    private void refreshTaskPanel() {
        ...
        btnSet.setActive(!tile.isSetup() && (curTask.getValue() instanceof IEUTask));
